 ...
 * 
 * Processes Lemon Squeezy webhook events and updates the database using the
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!userId && webhookData.event.includes("subscription_created")) {
    ...
    const planKey = webhookData.priceId
      ? getPlanFromVariantId(webhookData.priceId, "lemonsqueezy")
      : null
    const plan = planKey ? planKey.toLowerCase() : "free"

    if (webhookData.event.includes("subscription_created") || webhookData.event.includes("subscription_updated")) {
      if (!userId) {
        return NextResponse.json({ error: "No userId" }, { status: 400 })
      }
      ...
      if (userProfile?.email && plan !== "free" && webhookData.event.includes("subscription_created")) {
    ...
    } else if (webhookData.event.includes("subscription_cancelled")) {
